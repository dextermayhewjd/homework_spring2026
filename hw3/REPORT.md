...
1. **step 0 处 eval 反而更低（60 vs 211）。** 此时 Q 网络是随机初始化的。
   注意区分：随机的是**网络参数**，不是**策略** —— 「对随机网络取 argmax」得到的是一个
   **确定性但任意**的策略，相似状态永远选同一个动作。在 MsPacman 里它比均匀随机还差
   （后者至少会到处走动）。

   日志里有直接证据：

   | step | Eval_Avg | Eval_Min | Eval_Max |
   |---|---|---|---|
   | 0 | 60 | **60** | **60** |
   | 10,000 | 60 | **60** | **60** |
   | 20,000 | 164 | 110 | 330 |

   **前两个评估点的 10 条轨迹回报一个不差全是 60**，min = max = avg。
   若策略含随机性，10 次不可能同分 —— 何况 Atari 还有 `noop_max=30` 的随机起手，
   连初始状态都不相同。这个策略退化到了「不管看到什么都输出同一个动作、每次以同样方式死掉」。

   step 20,000 突然开始波动也有确切原因：`learning_starts=20000`，而 `run_dqn.py:129`
   的门槛是 `step >= learning_starts`。**在此之前 Q 网络一次梯度更新都没做过**，
   两次评估自然给出完全相同的结果。

   所以 eval 并非天然更高；它只是不用交探索税。早期它甚至输给随机策略。
