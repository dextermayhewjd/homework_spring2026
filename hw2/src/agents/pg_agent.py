...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        # return np.full(T,total,dtype=np.float32) 
        
        '''
        这里是两种写法两种都可以，sum是直接会把一整个数列的全部求和 可以直接吃一个生成表达式
        '''
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_reward_to_go(single_reward_traj) for single_reward_traj in rewards]
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            # 没有 critic 就没有任何 baseline 可减,优势估计退化成 Q 值本身。
        ...
        # advantages 此处已是一维 (batch_size,)，.mean()/.std() 不带参数即对整个 batch 求标量。
        # +1e-8 防除零：_discounted_return 会让同一条轨迹内所有值相同，
        # 万一某个 batch 只采到一条轨迹，std 就真的是 0。
