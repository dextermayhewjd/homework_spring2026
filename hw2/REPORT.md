...
全部 `seed=1`，run 目录形如 `LunarLander-v2_lunar_lander_lambda<λ>_sd1_20260821_172105`。

| λ | 训练中最高 Eval | 最终 Eval | 末 10 轮均值 | 首次 >150 的环境步数 | 末尾 Baseline Loss |
|---|---:|---:|---:|---:|---:|
| 0    | 97.97 | -60.32 | -67.06 | **未达到** | 3936.57 |
| 0.95 | 129.78 | -19.99 | -38.97 | **未达到** | 83.21 |
| 0.98 | 163.18 | -1.66 | 1.00 | 166,139 | 721.70 |
| **0.99** | **245.99** | **241.64** | **200.34** | 225,948 | 871.98 |
| 1    | 200.53 | 27.05 | 63.55 | 249,359 | 1810.97 |

**验收线满足**：λ=0.99 训练中最高 245.99 > 150。

> 加了「末 10 轮均值」和「首次 >150」两列（非 PDF 要求）：光看「最高 Eval」会误判 ——
> λ=0.98 和 λ=1 都摸到过 150+，但都没留住，只有 λ=0.99 末尾还在 200 以上。
>
> **末尾 Baseline Loss 这一列不可横向比较。** critic 的回归目标是 `q_values`（蒙特卡洛回报），
> **与 λ 无关** —— 五个 run 的 critic 学的是同一件事。数值差 47 倍是因为
> λ 改变策略 → 策略改变采到的轨迹 → 回报分布不同 → 同样的 MSE 不在一个量纲上。
> λ=0 的策略最差，LunarLander 坠毁的回报方差极大，所以目标量级和离散度都最大。
> 列出来只是为了说明这一点，不作为「critic 谁学得好」的证据。

### 3.2 图

五条 λ 曲线画在同一张（PDF 要求 "a single plot"）：

![LunarLander GAE λ 扫描学习曲线](report/exp3_lambda_sweep.png)

粗线为 10 点滑动平均，淡线为原始值 —— 五条噪声曲线直接叠加读不出趋势，
但原始值保留在图中，不隐藏波动。
...
**最优在 λ=0.99，两侧都变差，而且劣化方式不同** —— 这正是 bias-variance tradeoff 的形状，
不是「λ 越大越好」。

| λ | 表现 | 失败方式 |
|---|---|---|
| 0 | 全程在 -200 ~ -50，从未突破 0 | **根本上不去** —— 偏差主导 |
| 0.95 | 最高 129.78，末尾 -19.99 | 上不去 |
| 0.98 | 前 25 万步一路领先，30 万步后塌到 0 | **上得去，中途塌掉** |
| **0.99** | 30 万步后拉开，末段稳在 200 附近 | **上得去且稳住** ✓ |
| 1 | 中段稳定在 80–130，35 万步后缓慢下滑到 65 | **上得去，后期守不住** —— 方差主导 |

从图上看得很清楚：λ=0（蓝）自始至终在最底下；λ=0.98（绿）早期最快但后期崩塌；
λ=0.99（橙）是唯一一条末尾还在高位的；λ=1（粉）中段不错但末段掉头向下。

**机制**：λ 控制「用多少 bootstrap」。λ 小时 advantage 主要来自 $V_\phi$ 的单步预测，
$V_\phi$ 本身不准，这个偏差直接污染梯度方向 —— 所以 λ=0 学不到东西。
λ 大时几乎全用真实回报，无偏但方差大 —— LunarLander 的回报本就高方差（坠毁 vs 着陆
相差数百分），λ=1 的策略上得去却守不住。0.99 在两者之间取到了平衡。

**Q2. λ=0 对应什么？λ=1 对应什么？结合 LunarLander 的结果用一两句话说明。**

把 λ 代进递推 $A_t=\delta_t+\gamma\lambda A_{t+1}$：

$$\lambda=0:\quad A_t=\delta_t=r_t+\gamma V_\phi(s_{t+1})-V_\phi(s_t)$$

即 **PDF 式 (16) 的单步 TD 残差** —— 偏差最大、方差最小。

$$\lambda=1:\quad A_t=\sum_{t'=t}^{H-1}\gamma^{t'-t}\delta_{t'}
=\sum_{t'=t}^{H-1}\gamma^{t'-t}r_{t'}-V_\phi(s_t)$$

（中间 $V$ 项 telescoping 抵消）即**纯 Monte Carlo 回报减 baseline** —— 无偏、方差最大，
等价于实验 2 的 `-rtg + baseline`。

**本仓库的实现已对这两个极端做过钉桩验证**（见 `NOTES.md` 阶段 3「验证记录」）：
λ=0 时输出与手算的 $\delta$ 逐元素相等，λ=1 时与手算的 $Q-V$ 逐元素相等。

结合 LunarLander 的结果：**两个极端都不好，但坏法相反** —— λ=0 因为 $V_\phi$ 的偏差
被无限放大而始终学不起来（最高仅 97.97）；λ=1 因为高方差回报导致策略上得去却守不住
（最高 200.53，末尾只剩 27.05）。最优的 λ=0.99 恰好在两者之间。

### 3.4 完整命令

由 `run_experiments.sh exp3` 运行。`--no_gpu` 是相对 PDF 的唯一偏离
（实测 GPU 比 CPU 慢 1.7 倍，PDF §1 也推荐用 CPU）；`-blr` / `-bgs` 未指定，走默认 `5e-3` / `5`。

```bash
for LAM in 0 0.95 0.98 0.99 1; do
  uv run src/scripts/run.py --no_gpu --env_name LunarLander-v2 --ep_len 1000 --discount 0.99 \
    -n 200 -b 2000 -eb 2000 -l 3 -s 128 -lr 0.001 --use_reward_to_go --use_baseline \
    --gae_lambda $LAM --exp_name lunar_lander_lambda$LAM
done
```
